...
3. Set up a wind-cloud system using the *Haardt & Madau (2012)* metagalactic UV/X-ray background in `PLUTO`. This system has the current considerations:
   ...
   - There is a cooling floor at $10^4$ K (so we can consider ionised gas with $\sim$ constant mean molecular weight)
   - We include tables of both heating and cooling (generated with `py4radiation`) that are functions of hydrogen density and temperature. These tables span $\log_{10} n_{\rm H}\in \left[-9,\,4\right]$ cm$^{-3}$ and $\log_{10} T \in \left[1,\,9\right]$ K.
